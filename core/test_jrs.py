...
def test_net(cfg, epoch_idx=-1, test_data_loader=None, test_writer=None, model=None):
    ...
    if test_data_loader is None:
        # Set up data loader
        dataset_loader = utils.data_loaders.DATASET_LOADER_MAPPING[cfg.DATASET.TEST_DATASET](utils.data_loaders.DatasetSubset.VAL, cfg)
    ...
    with tqdm(test_data_loader) as t:
        ...
        for model_idx, (taxonomy_id, model_id, data, centroid, furthest_distance) in enumerate(t):
            ...
            with torch.no_grad():
                ...
                phi = 0.5
                cd1 = ((1 - phi) * chamfer(pcds[0], gt) + phi * chamfer(gt, pcds[0])).item() * 1e3
                cd2 = ((1 - phi) * chamfer(pcds[1], gt) + phi * chamfer(gt, pcds[1])).item() * 1e3
                cd3 = ((1 - phi) * chamfer(pcds[2], gt) + phi * chamfer(gt, pcds[2])).item() * 1e3
    ...
    if test_writer is not None:
        ...
        test_writer.add_scalar('Loss/Epoch/pmd', test_losses.avg(3), epoch_idx)
        for i, metric in enumerate(test_metrics.items):
            test_writer.add_scalar('Metric/%s' % metric, test_metrics.avg(i), epoch_idx)

    return test_losses.avg(2), test_losses.avg(3)
