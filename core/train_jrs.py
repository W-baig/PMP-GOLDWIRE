...
def train_net(cfg):
    ...
    train_dataset_loader = utils.data_loaders.DATASET_LOADER_MAPPING[cfg.DATASET.TRAIN_DATASET](utils.data_loaders.DatasetSubset.TRAIN, cfg)
    test_dataset_loader = utils.data_loaders.DATASET_LOADER_MAPPING[cfg.DATASET.TEST_DATASET](utils.data_loaders.DatasetSubset.VAL, cfg)

    train_data_loader = torch.utils.data.DataLoader(dataset=train_dataset_loader.get_dataset(
        utils.data_loaders.DatasetSubset.TRAIN),
                                                    batch_size=cfg.TRAIN.BATCH_SIZE, #16
    ...
    init_epoch = 0
    best_metrics = {'ChamferDistance':float('inf'), 'EmdDistance':float('inf')}
    ...
    for epoch_idx in range(init_epoch + 1, cfg.TRAIN.N_EPOCHS + 1):
        ...
        with tqdm(train_data_loader) as t:
            for batch_idx, (taxonomy_ids, model_ids, data, centroid, furthest_distance) in enumerate(t):
                ...
                phi = 0.5
                cd1 = (1 - phi) * chamfer(pcds[0], gt) + phi * chamfer(gt, pcds[0])
                cd2 = (1 - phi) * chamfer(pcds[1], gt) + phi * chamfer(gt, pcds[1])
                cd3 = (1 - phi) * chamfer(pcds[2], gt) + phi * chamfer(gt, pcds[2])
        ...
        # Save checkpoints
        if epoch_idx % cfg.TRAIN.SAVE_FREQ == 0 or cd_eval[0] < best_metrics['ChamferDistance']:   
            if cd_eval[0] < best_metrics['ChamferDistance']:
                best_metrics['ChamferDistance'] = cd_eval[0]
                best_metrics['EmdDistance'] = cd_eval[1]
                file_name = 'ckpt-best-%03d.pth' % epoch_idx 
            else :
