        ...
        for tr in self.transformers:
            ...
            for k, v in data.items():
                if k in objects and k in data:
                    ...
                    elif transform.__class__ == Normalize:
                        data[k], centroid, furthest_distance = transform(v)
                    else:
                        data[k] = transform(v)
...
class Normalize(object):
    ...
    def __call__(self, pc):
        ...
        centroid = np.mean(pc, axis=0)
        pc = pc - centroid  #centor
        furthest_distance = math.sqrt(np.max(np.sum(pc ** 2, axis=1)))
        pc = pc / furthest_distance  #(-1, 1)

        #further zoom  多除以2，其大小才和completion3d_dataset大小一致
        pc = pc / 2
...
class RandomClipPoints(object):
    def __init__(self, parameters):
        self.sigma = parameters['sigma'] if 'sigma' in parameters else 0.005
        self.clip = parameters['clip'] if 'clip' in parameters else 0.0025
...
class RandomRotatePoints(object):
    ...
    def __call__(self, ptcloud, rnd_value):
        ...
        axis = axis_choices[int(rnd_value * 3)]  # 随机选择一个轴
        
        # 计算旋转角度
        angle = 2 * math.pi * rnd_value
        
        # 创建旋转矩阵
        trfm_mat = transforms3d.axangles.axangle2mat(axis, angle)
